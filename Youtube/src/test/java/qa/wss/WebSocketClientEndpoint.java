...
public class WebSocketClientEndpoint {
	...
	/**
	 * Creates the client side endpoint and establish a session with the server's
	 * web socket addressed by the given URI.
	 * 
	 * @param serverEndpointURI The URI of the server's endpoint to establish a
	 *                          session via web sockets.
	 * @throws jakarta.websocket.DeploymentException 
	 */
	public WebSocketClientEndpoint(URI serverEndpointURI) throws IOException, DeploymentException, jakarta.websocket.DeploymentException {
		this.session = ClientManager.createClient().connectToServer(this, serverEndpointURI);
	}

	/**
	 * The onMessage method annotated with @OnMessage is called each time a message
	 * is received from server.
	 *
	 * @param message The message received from server.
	 * @param session The current session between the current client endpoint and
	 *                the server endpoint.
	 */
	@OnMessage
